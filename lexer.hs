...
import           Debug.Trace (trace, traceStack, traceShow)
...
data DFA = DFA { dfaAlphabet :: Set.Set Char
...
data Tokenizer = Tokenizer { tknrDFA :: DFA
                           , tknrID :: String
                           , tknrIgnore :: Bool
...
instance Show Tokenizer where
  show tokenizer =
    "Tokenizer " ++ tknrID tokenizer ++ " state " ++ show (tknrState tokenizer)

-- | Token represents a parsed token from a string. It contains the ID, and the start (inclusive)
-- and end (exclusive) indices.
data Token = Token { tokenID :: String, tokenStart :: Int, tokenEnd :: Int }
...
tknrNew alphabet startState deadState acceptStates transitionList tkid ignore =
  ...
  where
    verifiedDFA
      | invalidChars = error
        ("Invalid transition characters "
         ++ show invalidCharList
         ++ " for tokenizer "
         ++ tkid)
      | invalidTransitions = error
        ("Invalid transitions "
      ...
      | invalidStates = error
        ("Not all transitions defined for states"
         ...
         ++ "for tokenizer "
         ++ tkid)
      | otherwise = DFA { dfaAlphabet = alphabet
    ...
    expandedTransitions =
      let newTransitions = [((q, c), newDeadState)
                           | q <- fromStates
                           , c <- alphabetList
                           , (q, c) `Map.notMember` transitionMap]
          deadTransitions =
            [((newDeadState, c), newDeadState) | c <- alphabetList]
    ...
    invalidCharList =
      [x | ((_, x), _) <- transitionList, x `Set.notMember` alphabet]
    ...
    invalidStateTransitions =
      filter (\((_, _), qt) -> qt `notElem` fromStates) transitionList
...
tknrValidChar tokenizer char = char
  `Set.member` dfaAlphabet (tknrDFA tokenizer)
...
tknrStep
  tokenizer@Tokenizer { tknrState = maybeState
                      , tknrDFA = dfa
                      , tknrDeadState = deadState
                      }
  char = tokenizer { tknrState = dfaTransition
                       dfa
                       (fromMaybe (dfaStartState dfa) maybeState)
                       char
                       >>= checkDeadState
                   }
  where
...
_debugTokenize
  :: [Tokenizer] -> [Tokenizer] -> String -> [Token] -> Int -> Int -> String
...
_tokenizeInternal
  :: [Tokenizer] -> [Tokenizer] -> String -> [Token] -> Int -> Int -> [Token]
_tokenizeInternal a b c d e f
  | trace (_debugTokenize a b c d e f) False = undefined
...
_tokenizeInternal
  ...
  -- NO tokenizers are active: character is not valid
    ...
    | not (null matches) && tknrFinishedMatching matchedTokenizer =
      let
          ...
          newTokenizers =
            map (\tknr -> tknr { tknrState = Nothing }) tokenizers
          newToken = Token { tokenID = tknrID matchedTokenizer
          ...
          newTokens = if tknrIgnore matchedTokenizer
                      then tokens
                      else newToken:tokens
      in _tokenizeInternal
           newTokenizers
           []
           suffix
           newTokens
  ...
  where
    ...
    tknrValidNow tknr
      | endOfString = False
      | otherwise = let nextState = tknrState $ tknr `tknrStep` currentChar
                        dfa = tknrDFA tknr
...
tokenize tokenizers inputString =
  _tokenizeInternal tokenizers [] inputString [] 0 0
...
lexerFullAlphabet =
  lexerAlphanumeric `Set.union` lexerSymbols `Set.union` lexerWhitespace

commentTransitions =
  ((0, '/'), 1):((1, '/'), 2):[((2, x), 2) | x <- Set.toList lexerFullAlphabet]
...
whitespaceTokenizer =
  tknrNew lexerWhitespace 0 Nothing [1] whitespaceTransitions "WHITESPACE" True

commentTokenizer =
  tknrNew lexerFullAlphabet 0 Nothing [2] commentTransitions "COMMENT" False

main :: IO ()
main = do
  -- print (dfaAccept dfaDivBy4 "10100")
  let result = tokenize
        [commentTokenizer, plusTokenizer, minusTokenizer, whitespaceTokenizer]
        "++--/ /+-"
